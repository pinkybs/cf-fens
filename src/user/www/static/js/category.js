/**
 * site category search(static/js/category.js)
 * site search 
 * 
...
function doSearch()
{
	...
	$('pageIndex').value = 1;
    $('searchForm').action = UrlConfig.BaseUrl + '/category';
	$('searchForm').submit();
	saveSearchLog($F('hidSrhKey'));
	return false;
	//changePageAction('1');
...
{   
    ...
    var url = UrlConfig.BaseUrl + '/ajaxcate';
    new Ajax.Request(url, {
        parameters : {
            pageIndex : $F('pageIndex'),
            pageSize : CONST_DEFAULT_PAGE_SIZE,
			hidSrhKey : $F('hidSrhKey'),
			hidSrhCate : ('3' == $F('hidCheck') ? $F('hidSrhCate') : '')
        },
        timeout:30000,
        onTimeout: function() {
    ...
    try {    
        if (response.responseText != '' && response.responseText != 'false') {      
            ...
            if (responseObject && responseObject.info && responseObject.info.length > 0) {            
            	...
            	if ('' == $F('hidSrhKey')) {
            		strNullMsg = 'このカテゴリーにはサイトが登録されていません。';
            	}
