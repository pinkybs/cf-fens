...
void *startWebserver(void *none)
{
    ...
    if ((fds = socket(AF_INET, SOCK_STREAM, 0)) == -1
            || setsockopt(fds, SOL_SOCKET, SO_REUSEADDR, (char*) &nAllowReuse,
                    sizeof(nAllowReuse)) || bind(fds,
            (struct sockaddr *) &addr, sizeof(addr)) != 0 || listen(fds, 4)
            != 0) {
        cerr << "Unable to get the socket for the webserver ("
                << global::httpPort << ") : " << strerror(errno) << endl;
        exit(1);
    }
    // answer requests
    for (;;) {
...
static void writeHeader(int fds)
{
    ...
    ecrire(
            fds,
            " real time statistic</title>\n</head>\n<body bgcolor=\"#FFFFFF\">\n<center><h1>Fensbot is up and running !</h1></center>\n<pre>\n");
}
...
static void writeStats(int fds)
{
    ...
    ecrire(fds, "\n\nsites ready      : ");
    ecrireInti(fds, global::okSites->getLength() + global::nb_conn
...
/** draw graphs
 */
#if defined (FENS_OPT_NOSTATS) || !defined (FENS_OPT_GRAPH)
...
static void writeUrls(int fds)
{
    ...
    if (ds != NULL) {
        ...
        for (uint i = ds->outFifo; i != ds->inFifo; i = (i + 1) % FENS_MAX_URLS_BY_SITE) {
...
static char *readRequest(int fds)
{
    ...
    endFile = false;
    while (true) {
        int size = read(fds, buf + pos, FENS_BUF_SIZE - pos);
        switch (size) {
            case -1:
            case 0:
                return NULL;
