    ...
    if (reload) {
        ...
        if (fin == fout && fin != 0) {
            cerr << "previous crawl was too little, cannot reload state\n"
                    << "please restart fensbot with -scratch option\n";
            exit(1);
        }
        closedir(dir);
        in = (fin - fout) * FENS_URL_BY_FILE;
        out = 0;
        makeName(fin);
...
char *PersistentFifo::readLine()
{
    ...
    while (posn == NULL) {
        if (!(bufEnd - bufPos < FENS_MAX_URL_SIZE + 40 + 16 + FENS_MAX_URL_SIZE + FENS_MAX_COOKIE_SIZE)) {
            printf(fileName);
            printf(buf + bufPos);
        }
        if (bufPos * 2 > FENS_BUF_SIZE) {
            bufEnd -= bufPos;
        ...
        bool noRead = true;
        while (noRead) {
            int rd = read(rfds, buf + bufEnd, FENS_BUF_SIZE - 1 - bufEnd);
            switch (rd) {
                case 0:
                ...
                case -1:
                    ...
                    if (errno != EINTR) {
                        cerr
                                << "Big Problem while reading (persistentFifo.h)\n";
                        perror("reason");
                        assert(false);
                    } else {
...
{
    size_t len = strlen(s);
    assert(len < FENS_MAX_URL_SIZE + 40 + 16 + FENS_MAX_URL_SIZE + FENS_MAX_COOKIE_SIZE);
    if (outbufPos + len < FENS_BUF_SIZE) {
