...
#ifndef __FENS_GLOBAL_H__
#define __FENS_GLOBAL_H__
...
struct Connexion
{
    ...
    fens::FensString request; // what is the http request
    file *parser; // parser for the connexion (a robots.txt or an html file)
    char buffer[FENS_MAX_PAGE_SIZE];
    /** Constructor */
    Connexion();
...
struct global
{
    ...
    ~global();
    /* process number flag */
    static uint ProcessNum;
    /* process total number flag */
    static uint ProcessTotalNum;
    /* mysql connect config struct*/
    static MysqlConnectConfig mysqlConConfig;
    ...
    static uint nbDnsCNameChainsCalls;
    ...
    static void loadStartUrlsFromDB();
