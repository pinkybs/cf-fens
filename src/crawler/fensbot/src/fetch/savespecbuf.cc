...
void initSpecific()
{
    mkdir(FENS_SPEC_DIR, S_IRWXU);
    endFileName = strlen(FENS_SPEC_DIR);
    fileName = new char[endFileName + 20];
    strcpy(fileName, FENS_SPEC_DIR);
...
{
    nbfile++;
    if (nbfile >= FENS_FILES_PER_DIR) { // new dir
        nbdir++;
        nbfile = 0;
        ...
        if (indexFds < 0) {
            cerr << "cannot open file " << fileName << " : " << strerror(errno)
                    << endl;
            exit(1);
        }
    }
...
{
    static char content[FENS_MAX_SPEC_SIZE];
    if (mydir >= 0) {
        ...
        int pos = 0;
        while (cont) {
            cont = read(fds, content + pos, FENS_MAX_SPEC_SIZE - 1 - pos);
            pos += cont;
        }
        content[pos] = 0;
        close(fds);
