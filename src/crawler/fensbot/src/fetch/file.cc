...
/** Constructor
 */
robots::robots(NamedSite *server, Connexion *conn) :
    file(conn)
{
    newPars();
    this->server = server;
...
bool robots::parseHeaders()
{
    for (posParse = buffer + 9; posParse[3] != 0; posParse++) {
        if ((posParse[0] == '\n'
                && (posParse[1] == '\n' || posParse[2] == '\n'))
                || (posParse[0] == '\r' && (posParse[1] == '\r' || posParse[2]
                        == '\r'))) {
            return true;
        }
    }
    ...
    while (tok != NULL) {
        if (!strcasecmp(tok, "useragent") || !strcasecmp(tok, "user-agent")) {
            if (state == 2) {
                ...
                while (tok != NULL && strcasecmp(tok, "useragent")
                        && strcasecmp(tok, "user-agent") && strcasecmp(tok,
                        "disallow")) {
            ...
            tok = nextToken(&posParse, ':');
            while (tok != NULL && strcasecmp(tok, "useragent") && strcasecmp(
                    tok, "user-agent") && strcasecmp(tok, "disallow")) {
                // add nextToken to forbidden
                if (items++ < FENS_MAX_ROBOTS_ITEM) {
...
#if FENS_OPT_CGILEVEL >= 1
#define notCgiChar(c) (c!='?' && c!='=' && c!='*')
#else
#define notCgiChar(c) true
#endif // FENS_OPT_CGILEVEL
/** Constructor
 */
html::html(url *here, Connexion *conn) :
    file(conn)
{
    newPars();
    this->here = here;
    base = here->giveBase();
    headerCharset = new char[20];
    headerCharset[0] = '\0';
    state = ANSWER;
    isInteresting = false;
    constrSpec();
...
void html::manageUrl(url *nouv, bool isRedir)
{
    if (nouv->isValid() && filter1(nouv->getHost(), nouv->getFile())
            ...
            || this->here->isSameHost(nouv))) {
        // The extension is not stupid (gz, pdf...)
#ifdef FENS_OPT_LINKS_INFO
        links.addElement(nouv->giveUrl());
#endif // FENS_OPT_LINKS_INFO
        if (nouv->initOK(here)) {
            if (isRedir) {
                char *redirFrom = this->here->getRedirFrom();
                if (redirFrom == NULL) {
                    redirFrom = this->here->giveUrl();
                    nouv->setRedirFrom(redirFrom);
                    delete[] redirFrom;
                } else {
                    nouv->setRedirFrom(redirFrom);
                }
...
int html::parseHeader()
{
    if (posParse - area < 2) {
        // end of http headers
#ifndef FENS_OPT_FOLLOW_LINKS
        state = SPECIFIC;
#elif defined(FENS_OPT_SPECIFICSEARCH)
        if (isInteresting) {
            state = SPECIFIC;
        } else {
            state = HTML;
...
#ifdef FENS_OPT_ANYTYPE
#define checkType() return 0
#elif defined(FENS_OPT_IMAGES)
...
#else // FENS_OPT_SPECIFICSEARCH
            checkType()
            ;
#endif // FENS_OPT_SPECIFICSEARCH
        } else {
            //fixed space before charset
            int bk = 24;
            while (isspace(area[bk])) {
                bk++;
            }
            if (startWithIgnoreCase("charset=", area + bk)) { //Content-Type: text/html; charset=UTF-8
                bk += 8;
                int i = 0;
                while (!isspace(area[i + bk]) && notCgiChar(area[i+bk])) {
                    headerCharset[i] = area[i + bk];
                    i++;
                }
                if (notCgiChar(area[i+bk])) {
                    headerCharset[i] = '\0';
                }
            }
        }
...
{
#ifndef FENS_OPT_SPECIFICSEARCH
    if (startWithIgnoreCase("content-length: ", area)) {
        ...
        if (len > FENS_MAX_PAGE_SIZE) {
            printf("\nhtml::verifLength--tooBig\n");
            printf("\nlen:%d, FENS_MAX_PAGE_SIZE:%d\n", len, FENS_MAX_PAGE_SIZE);
            errno = tooBig;
            return 1;
        }
    }
    ...
    if (posParse - area < 2) {
        ...
        if (startWithIgnoreCase("location: ", area)) {
            ...
            if (notCgiChar(area[i])) {
                ...
                url *nouv = new url(here->getSiteId(), area+10, here->getDepth(), base);
#else
...
void html::parseContent(int action)
{
    ...
    if (*posParse == '\"' || *posParse == '\'')
        posParse++;
    area = posParse;
    char *endItem = area + FENS_MAX_URL_SIZE;
...
#ifdef FENS_OPT_URL_SITEID
                manageUrl(new url(here->getSiteId(), area, here->getDepth()-1, base), false);
#else
