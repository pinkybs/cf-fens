...
void checkTimeout()
{
    for (uint i = 0; i < global::nb_conn; i++) {
        ...
        if (conn->state != emptyC) {
            if (conn->timeout > 0) {
                if (conn->timeout > FENS_TIMEOUT_PAGE) {
...
static void pipeRead(Connexion *conn)
{
    ...
    int size = read(conn->socket, conn->buffer + p, FENS_MAX_PAGE_SIZE - p - 1);
    switch (size) {
        case 0:
            // End of file
        ...
        default:
            ...
            conn->timeout += size / FENS_TIMEOUT_INCR;
            addRead(size);
#ifdef FENS_OPT_MAX_BANDWIDTH
            ...
            if (conn->parser->inputHeaders(size) == 0) {
                // nothing special
                if (conn->parser->pos >= FENS_MAX_PAGE_SIZE - 1) {
                    // We've read enough...
                    conn->err = tooBig;
                    endOfFile(conn);
                }
