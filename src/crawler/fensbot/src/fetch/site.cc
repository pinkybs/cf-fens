...
{
    fifo[inFifo] = u;
    inFifo = (inFifo + 1) % FENS_MAX_URLS_BY_SITE;
...
url *NamedSite::getInFifo()
{
    ...
    outFifo = (outFifo + 1) % FENS_MAX_URLS_BY_SITE;
    return tmp;
}

short NamedSite::fifoLength()
{
    return (inFifo + FENS_MAX_URLS_BY_SITE - outFifo) % FENS_MAX_URLS_BY_SITE;
}

/* Put an url in the fifo if their are not too many */
void NamedSite::putGenericUrl(url *u, int limit, bool prio)
{
    if (nburls > FENS_MAX_URLS_BY_SITE - limit) {
...
void NamedSite::dnsAns(adns_answer *ans)
{
    if (ans->status == adns_s_prohibitedcname) {
        if (cname == NULL) {
            ...
            printf("\nCalls[%d]: CNAME: %s, ANSWER CNAME: %s\n",
                    global::nbDnsCNameChainsCalls, cname, ans->cname);
            if (global::nbDnsCNameChainsCalls < global::dnsCNameChains) {
                cname = newString(ans->cname);
                global::nbDnsCalls++;
                global::nbDnsCNameChainsCalls++;
                adns_query quer = NULL;
                adns_submit(global::ads, cname, (adns_rrtype) adns_r_addr,
                        (adns_queryflags) 0, this, &quer);
            } else {
                global::nbDnsCNameChainsCalls = 0;
                siteSeen();
                delete[] cname;
                cname = NULL;
                dnsState = errorDns;
        ...
        if (ans->status != adns_s_ok) {
            // No addr inet
            //printf("\n dns status: %d, adns_s_querydomaininvalid: %d\n", ans->status, adns_s_querydomaininvalid);
            dnsState = errorDns;
            dnsErr();
        } else {
            siteDNS();
...
void NamedSite::dnsOK()
{
    ...
    if (res != emptyC) {
        conn->timeout = FENS_TIMEOUT_PAGE;
        if (global::proxyAddr != NULL) {
            // use a proxy
...
void NamedSite::newId()
{
    ...
    port = u->getPort();
    dnsTimeout = global::now + FENS_DNS_VALID_TIME;
    dnsState = waitDns;
}
...
inline url *IPSite::getUrl()
{
    ...
    global::inter->getOne();
#if defined(FENS_OPT_SPECIFICSEARCH) && !defined(FENS_OPT_NOSTATS)
