...
static Input *inputConns[FENS_MAX_INPUT];
...
int input()
{
    ...
    int n = -1;
    if (nbInput < FENS_MAX_INPUT - 1 && global::ansPoll[inputFds]) {
...
static bool readMore(Input *in)
{
    ...
    if (in->end_posp - in->pos > FENS_MAX_URL_SIZE + 100) {
        // error -> stop connection
        ecrire(in->fds, "Url submitted too long\n");
        in->priority = END;
        return false;
    }
    if (2 * in->pos > FENS_BUF_SIZE) {
        in->end_pos -= in->pos;
        in->end_posp = in->end_pos;
        memmove(in->buffer, in->buffer + in->pos, in->end_pos);
        in->pos = 0;
    }
    int nb = read(in->fds, in->buffer + in->end_pos, FENS_BUF_SIZE - in->end_pos);
...
void initInput()
{
    if (global::inputPort != 0) {
        ...
        if ((inputFds = socket(AF_INET, SOCK_STREAM, 0)) == -1 || setsockopt(
                inputFds, SOL_SOCKET, SO_REUSEADDR, (char*) &allowReuse,
                sizeof(allowReuse)) || bind(inputFds,
        ...
        for (int i = 0; i < FENS_MAX_INPUT; i++) {
