...
void loaded(html *page)
{
    ...
    char *f = u->getFile();
    // update dir name
    uint d = u->hostHashCode() % FENS_NB_DIR;
    for (int i = 2; i < 7; i++) {
    ...
    while (cont) {
        ...
        while (fileName[len] != '/')
            len--;
        fileName[len] = 0;
        cont = stat(fileName, &st); // this becomes true at least for FENS_SAVE_DIR
        fileName[len] = '/';
    }
    cont = true;
    while (cont) {
    ...
    if (fileName[len - 1] == '/') {
        strcpy(fileName + len, FENS_INDEX_FILE);
    }
...
void initUserOutput()
{
    mkdir(FENS_SAVE_DIR, S_IRWXU);
    endFileName = strlen(FENS_SAVE_DIR);
    fileName = new char[endFileName + FENS_MAX_URL_SIZE + 50];
    strcpy(fileName, FENS_SAVE_DIR);
